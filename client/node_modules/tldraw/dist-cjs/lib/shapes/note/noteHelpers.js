...
__export(noteHelpers_exports, {
  ...
  NOTE_PIT_RADIUS: () => NOTE_PIT_RADIUS,
  NOTE_SIZE: () => NOTE_SIZE,
...
const NOTE_SIZE = 200;
const NOTE_CENTER_OFFSET = { x: NOTE_SIZE / 2, y: NOTE_SIZE / 2 };
const NOTE_PIT_RADIUS = 10;
const DEFAULT_PITS = {
...
function getNoteShapeForAdjacentPosition(editor, shape, center, pageRotation, forceNew = false) {
  ...
  if (!nextNote || forceNew) {
    ...
    const topLeft = editor.getPointInParentSpace(
      createdShape,
      import_editor.Vec.Sub(center, import_editor.Vec.Rot(NOTE_CENTER_OFFSET, pageRotation))
    );
    editor.updateShape({
      id,
      type: "note",
