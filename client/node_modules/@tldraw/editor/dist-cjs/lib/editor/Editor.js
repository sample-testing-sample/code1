...
class Editor extends import_eventemitter3.EventEmitter {
  ...
  duplicateShapes(shapes, offset) {
    ...
    const shapesToCreate = (0, import_utils.compact)(
      idsToCreate.map((id) => {
        ...
        let newShape = (0, import_utils.structuredClone)(shape);
  ...
  putContentOntoCurrentPage(content, options = {}) {
    ...
    const newShapes = shapes.map((shape) => {
      let newShape;
      if (preserveIds) {
        newShape = (0, import_utils.structuredClone)(shape);
        idMap.set(shape.id, shape.id);
      } else {
        const id = idMap.get(shape.id);
        newShape = (0, import_utils.structuredClone)({ ...shape, id });
      }
      ...
      case "wheel": {
        ...
        if (this.getIsMenuOpen()) {
          ...
          if (inputs.ctrlKey) {
            ...
            const zoom = Math.min(import_constants.MAX_ZOOM, Math.max(import_constants.MIN_ZOOM, cz2 + (info.delta.z ?? 0) * cz2));
            this._setCamera(
              {
          ...
          if (!inputs.isDragging && inputs.isPointing && import_Vec.Vec.Dist2(originPagePoint, currentPagePoint) > (this.getInstanceState().isCoarsePointer ? import_constants.COARSE_DRAG_DISTANCE : import_constants.DRAG_DISTANCE) / this.getZoomLevel()) {
            clearTimeout(this._longPressTimeout);
      ...
      case "pointer": {
        ...
        switch (info.name) {
          case "pointer_down": {
            ...
            }, import_constants.LONG_PRESS_DURATION);
            this._selectedShapeIdsAtPointerDown = this.getSelectedShapeIds();
            if (info.button === 0) {
