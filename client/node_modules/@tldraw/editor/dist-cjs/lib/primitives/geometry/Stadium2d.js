  ...
  getVertices() {
    ...
    if (h > w) {
      for (let i = 0; i < STADIUM_VERTICES_LENGTH - 1; i++) {
        t1 = -import_utils.PI + import_utils.PI * i / (STADIUM_VERTICES_LENGTH - 2);
        t2 = import_utils.PI * i / (STADIUM_VERTICES_LENGTH - 2);
        points[i] = new import_Vec.Vec(cx + cx * Math.cos(t1), cx + cx * Math.sin(t1));
        points[i + (STADIUM_VERTICES_LENGTH - 1)] = new import_Vec.Vec(
          cx + cx * Math.cos(t2),
          h - cx + cx * Math.sin(t2)
        );
      }
      ...
      for (let i = 0; i < STADIUM_VERTICES_LENGTH - 1; i++) {
        t1 = -import_utils.HALF_PI + import_utils.PI * i / (STADIUM_VERTICES_LENGTH - 2);
        t2 = import_utils.HALF_PI + import_utils.PI * -i / (STADIUM_VERTICES_LENGTH - 2);
